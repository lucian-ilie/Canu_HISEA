...
sub merylProcess ($$$) {
    ...
    if ((getGlobal("${tag}Overlapper") eq "hisea") &&
        (! -e $ffile)) {
        ...
        my $filterThreshold = (getGlobal("${tag}HiseaSensitivity") eq "normal") ?   getGlobal("${tag}HiseaFilterThreshold") :   getGlobal("${tag}HiseaFilterThreshold");  #  Also set in Meryl.pm

        my $misRate  = 0.1;
        my $minCount = defined(getGlobal("${tag}HiseaFilterUnique")) ? uniqueKmerThreshold($wrk, $asm, $merSize, $misRate)+1 : int($filterThreshold * $totalMers);
        if ($minCount > 10000)
        {
            $minCount=10000;
        }
        ...
        if (defined(getGlobal("${tag}HiseaFilterUnique"))) {
           printf STDERR "-- For %s overlapping, filtering low-occurence k-mers < %d (%.2f\%) based on estimated error of %.2f\%.\n", getGlobal("${tag}Overlapper"), $minCount, $totalFiltered, 100*estimateRawError($wrk, $asm, $tag, $merSize);
        }
        printf STDERR "-- For %s overlapping, set repeat k-mer threshold to %d.\n", getGlobal("${tag}Overlapper"), int($filterThreshold * $totalMers);
    }
