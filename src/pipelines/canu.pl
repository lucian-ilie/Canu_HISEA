...
sub overlap ($$$) {
    ...
    if (getGlobal("${tag}overlapper") eq "mhap") {
        ...
        hiseaConfigure($wrk, $asm, $tag, $ovlType);

        hiseaPrecomputeCheck($wrk, $asm, $tag, $ovlType)  foreach (1..getGlobal("canuIterationMax") + 1);

        #  this also does hiseaReAlign

        hiseaCheck($wrk, $asm, $tag, $ovlType)  foreach (1..getGlobal("canuIterationMax") + 1);
