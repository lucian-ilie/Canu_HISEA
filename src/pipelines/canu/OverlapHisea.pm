...
package canu::OverlapHisea;

require Exporter;

@ISA    = qw(Exporter);
@EXPORT = qw(hiseaConfigure hiseaPrecomputeCheck hiseaCheck);
...
sub hiseaConfigure ($$$$) {
    ...
    goto allDone   if (skipStage($WRK, $asm, "$tag-hiseaConfigure") == 1);
    goto allDone   if (-e "$path/precompute.sh") && (-e "$path/hisea.sh");
    goto allDone   if (-e "$path/ovljob.files");
    goto allDone   if (-e "$wrk/$asm.ovlStore");

    print STDERR "--\n";
    print STDERR "-- OVERLAPPER (hisea) (correction)\n"  if ($tag eq "cor");
    print STDERR "-- OVERLAPPER (hisea) (trimming)\n"    if ($tag eq "obt");
    print STDERR "-- OVERLAPPER (hisea) (assembly)\n"    if ($tag eq "utg");
    print STDERR "--\n";
    ...
    if (!defined(getGlobal("${tag}HiseaSensitivity"))) {
        my $cov = getExpectedCoverage($wrk, $asm);

        setGlobal("${tag}HiseaSensitivity", "low");                          #  Yup, super inefficient.  The code is
        setGlobal("${tag}HiseaSensitivity", "normal")   if ($cov <  60);     #  compact and clear and runs once.
        setGlobal("${tag}HiseaSensitivity", "high")     if ($cov <= 30);     #  Live with it.

        print STDERR "-- Set ${tag}HiseaSensitivity=", getGlobal("${tag}HiseaSensitivity"), " based on read coverage of $cov.\n";
    }

    if      (getGlobal("${tag}HiseaSensitivity") eq "low") {
        $numHashes     =  256;
        $minNumMatches =   10;
        $ordSketch     = 3000; # Used for maxKmerHits

    } elsif (getGlobal("${tag}HiseaSensitivity") eq "normal") {
        $numHashes     =  512;
        $minNumMatches =    5;
        $ordSketch     = 3000; # Used for maxKmerHits

    } elsif (getGlobal("${tag}HiseaSensitivity") eq "high") {
        $numHashes     =  768;
        $minNumMatches =    3;
        $ordSketch     = 3000; # Used for maxKmerHits
        #$ordSketchMer  = getGlobal("${tag}HiseaOrderedMerSize");

    } else {
        caFailure("invalid ${tag}HiseaSensitivity=" . getGlobal("${tag}HiseaSensitivity"), undef);
    }

    my $filterThreshold = getGlobal("${tag}HiseaFilterThreshold");

    #  Constants.

    my $merSize       = getGlobal("${tag}HiseaMerSize");

    my $numReads      = getNumberOfReadsInStore($wrk, $asm);
    my $memorySize    = getGlobal("${tag}hiseaMemory");
    my $blockPerGb    = getGlobal("${tag}HiseaBlockSize");
    #if ($numHashes >= 768) {
    ...
    print F "fi\n";
    print F "\n";
    print F "echo Starting hisea precompute.\n";
    print F "\n";
    print F "#  So hisea writes its output in the correct spot.\n";
    ...
    #print F "|| \\\n";
    print F "  \$bin/hisea", "\\\n";
    print F "  --kmerLen $merSize \\\n";
    print F "  --minMatches $minNumMatches \\\n";
    print F "  --filterCount 2 \\\n"  if (defined(getGlobal("${tag}HiseaFilterUnique")) && getGlobal("${tag}HiseaFilterUnique") == 1);
    print F "  --filterCount 1 \\\n"  if (!defined(getGlobal("${tag}HiseaFilterUnique")) || (defined(getGlobal("${tag}HiseaFilterUnique")) && getGlobal("${tag}HiseaFilterUnique") != 1));
    print F "  --maxKmerHits $ordSketch \\\n";
    print F "  --threads ", getGlobal("${tag}hiseaThreads"), " \\\n";
    print F "  --ignore $wrk/0-mercounts/$asm.ms$merSize.frequentMers.ignore.gz", "\\\n"   if (-e "$wrk/0-mercounts/$asm.ms$merSize.frequentMers.ignore.gz");
    print F "  --ref $path/blocks/\$job.fasta", "\\\n";
    print F "  --index_write $path/blocks",  "\\\n";
    print F "|| \\\n";
    print F "mv -f $path/blocks/\$job.dat $path/blocks/\$job.dat.FAILED\n";
    print F "\n";
    print F "if [ ! -e \"$path/blocks/\$job.dat\" ] ; then\n";
    print F "  echo Hisea failed.\n";
    print F "  exit 1\n";
    print F "fi\n";
    print F "\n";
    ...
    #  Create a script to run hisea.

    open(F, "> $path/hisea.sh") or caFailure("can't open '$path/hisea.sh' for writing: $!", undef);
    ...
    print F getBinDirectoryShellCode();
    print F "\n";
    print F "if [ ! -e \"$path/results/\$qry.hisea\" ] ; then\n";
    print F "    \$bin/hisea", "\\\n";
    print F "    --kmerLen $merSize \\\n";
    print F "    --minMatches $minNumMatches \\\n";
    print F "    --filterCount 1 \\\n";
    ...
    print F "    --threads ", getGlobal("${tag}hiseaThreads"), " \\\n";
    print F "    --ignore $wrk/0-mercounts/$asm.ms$merSize.frequentMers.ignore.gz", "\\\n"   if (-e "$wrk/0-mercounts/$asm.ms$merSize.frequentMers.ignore.gz");
    print F "    --ref $path/blocks/\$blk.fasta \$slf", "\\\n";
    print F "    --index_read $path/blocks",  "\\\n";
    print F "    --query $path/queries/\$qry", "\\\n";
    print F "  > $path/results/\$qry.hisea.WORKING \\\n";
    print F "  && \\\n";
    print F "  mv -f $path/results/\$qry.hisea.WORKING $path/results/\$qry.hisea\n";
    print F "fi\n";
    print F "\n";

    print F "if [   -e \"$path/results/\$qry.hisea\" -a \\\n";
    print F "     ! -e \"$path/results/\$qry.ovb.gz\" ] ; then\n";
    print F "  \$bin/hiseaConvert \\\n";
    print F "    \$cvt \\\n";
    print F "    -o $path/results/\$qry.hisea.ovb.WORKING.gz \\\n";
    print F "    $path/results/\$qry.hisea \\\n";
    print F "  && \\\n";
    print F "  mv $path/results/\$qry.hisea.ovb.WORKING.gz $path/results/\$qry.hisea.ovb.gz\n";
    print F "fi\n";
    print F "\n";

    if (getGlobal('saveOverlaps') eq "0") {
        print F "if [   -e \"$path/results/\$qry.hisea\" -a \\\n";
        print F "       -e \"$path/results/\$qry.hisea.ovb.gz\" ] ; then\n";
        print F "  rm -f $path/results/\$qry.hisea\n";
        print F "fi\n";
        print F "\n";
    }

    print F "if [ -e \"$path/results/\$qry.hisea.ovb.gz\" ] ; then\n";
    if (getGlobal("${tag}ReAlign") eq "raw") {
        print F "  \$bin/overlapPair \\\n";
        print F "    -G $wrk/$asm.gkpStore \\\n";
        print F "    -O $path/results/\$qry.hisea.ovb.gz \\\n";
        ...
        print F "    -memory " . getGlobal("${tag}hiseaMemory") . " \\\n";
        print F "    -t " . getGlobal("${tag}hiseaThreads") . " \n";
    } else {
        print F "  mv -f \"$path/results/\$qry.hisea.ovb.gz\" \"$path/results/\$qry.ovb.gz\"\n";
    }
    print F "fi\n";
    ...
    if (-e "$path/precompute.sh") {
        ...
        print STDERR "-- Configured $numJobs hisea precompute jobs.\n";
    }

    if (-e "$path/hisea.sh") {
        my $numJobs = 0;
        open(F, "< $path/hisea.sh") or caFailure("can't open '$path/hisea.sh' for reading: $!", undef);
        while (<F>) {
            $numJobs++  if (m/^\s+qry=\"(\d+)\"$/);
        }
        close(F);

        print STDERR "-- Configured $numJobs hisea overlap jobs.\n";
    }

  finishStage:
    emitStage($WRK, $asm, "$tag-hiseaConfigure");
    buildHTML($WRK, $asm, $tag);
    stopAfter("hiseaConfigure");
...
sub hiseaPrecomputeCheck ($$$$) {
    ...
    goto allDone   if (skipStage($WRK, $asm, "$tag-hiseaPrecomputeCheck", $attempt) == 1);
    ...
    if (scalar(@failedJobs) > 0) {
        ...
        if ($attempt > 1) {
            print STDERR "--\n";
            print STDERR "-- ", scalar(@failedJobs), " hisea precompute jobs failed:\n";
        ...
        if ($attempt > getGlobal("canuIterationMax")) {
            caExit("failed to precompute hisea indices.  Made " . ($attempt-1) . " attempts, jobs still failed", undef);
        }

        #  Otherwise, run some jobs.

        print STDERR "-- hisea precompute attempt $attempt begins with ", scalar(@successJobs), " finished, and ", scalar(@failedJobs), " to compute.\n";

        emitStage($WRK, $asm, "$tag-hiseaPrecomputeCheck", $attempt);
        buildHTML($WRK, $asm, $tag);

        submitOrRunParallelJob($WRK, $asm, "${tag}hisea", $path, "precompute", @failedJobs);
        return;
    }

  finishStage:
    print STDERR "-- All ", scalar(@successJobs), " hisea precompute jobs finished successfully.\n";
    ...
    emitStage($WRK, $asm, "$tag-hiseaPrecomputeCheck");
    buildHTML($WRK, $asm, $tag);
    stopAfter("hiseaPrecompute");
...
sub hiseaCheck ($$$$) {
    ...
    goto allDone   if (skipStage($WRK, $asm, "$tag-hiseaCheck", $attempt) == 1);
    goto allDone   if (-e "$path/hisea.files");
    goto allDone   if (-e "$wrk/$asm.ovlStore");
    ...
    open(F, "< $path/hisea.sh") or caExit("failed to open '$path/hisea.sh'", undef);
    while (<F>) {
        if (m/^\s+qry=\"(\d+)\"$/) {
            if      (-e "$path/results/$1.ovb.gz") {
                push @hiseaJobs,    "$path/results/$1.hisea\n";
    ...
    if (scalar(@failedJobs) > 0) {
        ...
        if ($attempt > 1) {
            print STDERR "--\n";
            print STDERR "-- ", scalar(@failedJobs), " hisea jobs failed:\n";
            print STDERR $failureMessage;
            print STDERR "--\n";
        }
        ...
        if ($attempt > getGlobal("canuIterationMax")) {
            caExit("failed to compute hisea overlaps.  Made " . ($attempt-1) . " attempts, jobs still failed", undef);
        }

        #  Otherwise, run some jobs.

        print STDERR "-- hisea attempt $attempt begins with ", scalar(@successJobs), " finished, and ", scalar(@failedJobs), " to compute.\n";

        emitStage($WRK, $asm, "$tag-hiseaCheck", $attempt);
        buildHTML($WRK, $asm, $tag);
        submitOrRunParallelJob($WRK, $asm, "${tag}hisea", $path, "hisea", @failedJobs);
        return;
    }

  finishStage:
    print STDERR "-- Found ", scalar(@successJobs), " hisea overlap output files.\n";

    open(L, "> $path/hisea.files") or caExit("failed to open '$path/hisea.files'", undef);
    print L @hiseaJobs;
    close(L);
    ...
    setGlobal("canuIteration", 0);
    emitStage($WRK, $asm, "$tag-hiseaCheck");
    buildHTML($WRK, $asm, $tag);
    stopAfter("hiseaOverlap");
