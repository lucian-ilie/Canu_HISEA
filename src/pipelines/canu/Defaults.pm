...
sub setGlobal ($$) {
    ...
    foreach my $alg ("ovl", "mhap", "mmap", "hisea") {
    ...
    #  e.g., corHiseaBlockSize
    foreach my $opt ("hiseablocksize", "hiseamersize", "hisearealign", "hiseasensitivity") {
        $set += setGlobalSpecialization($val, ("cor${opt}", "obt${opt}", "utg${opt}"))  if ($var eq "${opt}");
    }
...
#    $synops{"${tag}MMapBlockSize"}            = "Number of reads per 1GB; memory * blockSize = the size of  block loaded into memory per job";
    ...
    $global{"${tag}HiseaFilterThreshold"}      = "0.000005";
    $synops{"${tag}HiseaFilterThreshold"}      = "Value between 0 and 1. kmers which comprise more than this percentage of the input are downweighted";

    $global{"${tag}HiseaFilterUnique"}         = undef;
    $synops{"${tag}HiseaFilterUnique"}         = "Expert option: True or false, supress the low-frequency k-mer distribution based on them being likely noise and not true overlaps. Threshold auto-computed based on error rate and coverage.";

#    $global{"${tag}MhapNoTf"}                 = undef;
#    $synops{"${tag}MhapNoTf"}                 = "Expert option: True or false, do not use tf weighting, only idf of tf-idf.";

    $global{"${tag}HiseaBlockSize"}            = 3000;
    $synops{"${tag}HiseaBlockSize"}            = "Number of reads per 1GB; memory * blockSize = the size of  block loaded into memory per job";

    $global{"${tag}HiseaMerSize"}              = ($tag eq "cor") ? 16 : 22;
    $synops{"${tag}HiseaMerSize"}              = "K-mer size for seeds in hisea";

    #$global{"${tag}HiseaOrderedMerSize"}       = ($tag eq "cor") ? 12 : 22;
    #$synops{"${tag}HiseaOrderedMerSize"}       = "K-mer size for second-stage filter in hisea";

    $global{"${tag}HiseaSensitivity"}          = undef;
    $synops{"${tag}HiseaSensitivity"}          = "Coarse sensitivity level: 'low', 'normal' or 'high'.  Usually set automatically based on coverage; 'high' <= 30x < 'normal' < 60x <= 'low'";
...
sub setDefaults () {
    ...
    foreach my $c (qw(BAT CNS COR MERYL CORMHAP CORMMAP COROVL CORHISEA OBTMHAP OBTOVL OBTHISEA OEA OVB OVS RED UTGMHAP UTGMMAP UTGOVL UTGHISEA)) {
        $global{"useGrid$c"} = 1;
    ...
    setOverlapDefaults("cor", "correction",             "hisea");  #  Overlaps computed for correction
...
sub checkParameters () {
    ...
    foreach my $tag ("cor", "obt", "utg") {
        if ((getGlobal("${tag}Overlapper") ne "mhap") &&
            ...
            addCommandLineError("ERROR:  Invalid '${tag}Overlapper' specified (" . getGlobal("${tag}Overlapper") . "); must be 'mhap', 'hisea', 'ovl', or 'minimap'\n");
        }
    }
    ...
    foreach my $tag ("cor", "obt", "utg") {
        ...
        if (getGlobal("${tag}HiseaSensitivity") eq "fast") {
            print STDERR "WARNING: deprecated ${tag}HiseaSensitivity=fast replaced with ${tag}HiseaSensitivity=low\n";
        }

        if ((getGlobal("${tag}HiseaSensitivity") ne undef)    &&
            (getGlobal("${tag}HiseaSensitivity") ne "low")    &&
            (getGlobal("${tag}HiseaSensitivity") ne "normal") &&
            (getGlobal("${tag}HiseaSensitivity") ne "high")) {
            addCommandLineError("ERROR:  Invalid '${tag}HiseaSensitivity' specified (" . getGlobal("${tag}HiseaSensitivity") . "); must be 'fast' or 'normal' or 'high'\n");
        }
    }
