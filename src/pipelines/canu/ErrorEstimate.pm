...
sub computeSampleSize($$$$$) {
    ...
    if (defined("mhap"))
    {
        $maxSampleSize = getGlobal("${tag}MhapBlockSize") * 4;
    }

    if (defined("hisea"))
    {
        $maxSampleSize = getGlobal("${tag}HiseaBlockSize") * 4;
    }
...
sub runHISEA($$$$$$$$$$$$) {
    ...
    my $cmd  = "$bin/hisea";
    $cmd .= "  --filterCount 1 --kmerLen $merSize --minMatches $minNumMatches --similarity $threshold --kmerFilter $filterThreshold --threads " . getGlobal("${tag}hiseaThreads");
    $cmd .= " --ref $hash --query $query  2> /dev/null | awk '{if (\$1 != \$2+$sampleSize) { print \$0}}' | $bin/errorEstimate -d 2 -m 0.95 -S - > $out 2> $err";
    runCommand($wrk, $cmd);
}
sub runMHAP($$$$$$$$$$$$) {
    my ($wrk, $tag, $numHashes, $minNumMatches, $threshold, $ordSketch, $ordSketchMer, $sampleSize, $hash, $query, $out, $err) = @_;
...
sub estimateRawError($$$$) {
    ...
    if (defined("hisea")) 
    {
        print STDERR "-- ESTIMATOR (hisea) (raw) (hash sample size=". ($sampleSize*2) . ") (query sample size=$querySize)\n";
        runHISEA($wrk, $tag, $numHashes, $minNumMatches, $threshold, $ordSketch, $ordSketchMer, $sampleSize*2, "$wrk/$asm.gkpStore/subset.fasta", "$wrk/$asm.gkpStore/reads.fasta", "$wrk/$asm.gkpStore/raw.estimate.out", "$wrk/$asm.gkpStore/raw.estimate.err");
    }
...
sub estimateCorrectedError ($$$) {
    ...
    if (defined("hisea"))
    {
        $ordSketchMer  	    = getGlobal("${tag}HiseaOrderedMerSize") + 2;
    }
    ...
    if (defined("hisea"))
    {
        print STDERR "-- ESTIMATOR (hisea) (corrected) (hash sample size=$sampleSize) (query sample size=$querySize)\n";

        runHISEA($wrk, $tag, $numHashes, $minNumMatches, $threshold, $ordSketch, $ordSketchMer, $sampleSize, "$path/subset.fasta", "$path/reads.fasta", "$path/$asm.estimate.out", "$path/$asm.estimate.err");
    }
